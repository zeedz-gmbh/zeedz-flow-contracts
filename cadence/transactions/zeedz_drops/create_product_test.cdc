...
transaction(name: String, description: String, id: String, total: UInt64, saleEnabled: Bool) {
    ...
    execute {
        ...
        self.dropsAdmin.addProduct(name: name, description: description, id: id, total: total, saleEnabled: saleEnabled, timeStart: getCurrentBlock().timestamp-(600.0*10.0), timeEnd: getCurrentBlock().timestamp+(600.0*10.0), prices: prices)
    }
}